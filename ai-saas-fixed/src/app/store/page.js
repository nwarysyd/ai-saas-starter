...
export default function AppStore() {
  ...
  const handleInstallApp = async (app) => {
    ...
    try {
      await axios.post("/api/installed-apps", { appId: app.id });
      toast.success(`${app.name} installed successfully! 🎉`);
    } catch (err) {
      const msg = err.response?.data?.error || "Installation failed";
      if (msg.includes("already installed")) {
        toast.error(`${app.name} is already installed`);
      } else {
        toast.error(msg);
      }
    } finally {
                    ...
                    >
                      ...
                      <div className="p-4 space-y-3">
                        ...
                        <button
                          ...
                          className="w-full py-2 px-3 bg-primary hover:bg-primary-hover disabled:opacity-60 text-white font-bold rounded transition-all text-sm active:scale-95"
                        >
